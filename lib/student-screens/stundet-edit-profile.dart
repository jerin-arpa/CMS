...
class StudentEditProfile extends StatefulWidget {
  static String id = 'student-edit-profile';
...
class _StudentEditProfileState extends State<StudentEditProfile> {
  ...
  void getData()async{
   await FirebaseFirestore.instance.collection('studentProfile').where('email', isEqualTo: user?.email).get().then((value){
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () {
          _firestore.collection('studentProfile').doc(Provider.of<TaskData>(context, listen: false).userEmail).update({
            'batch': batch,
            'dept': _dropDownValue,
            'bio': bio,
            'mobile': mobile,
